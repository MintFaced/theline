    ...
    <div className="bg-line-bg min-h-screen" style={{ paddingTop: 'var(--nav-height)' }}>
      ...
      <RevealSection className="border-b border-line-border">
        <div className="grid md:grid-cols-3 gap-px bg-line-border">
          ...
          {/* Big featured */}
          <Link href={`/storyline/${featured.slug}`} className="md:col-span-2 block group relative overflow-hidden" style={{ minHeight: '480px' }}>
          ...
          <div className="flex flex-col gap-px bg-line-border">
            {next2.map((post: any) => (
              <Link key={post.slug} href={`/storyline/${post.slug}`} className="bg-line-bg group flex flex-col flex-1 overflow-hidden relative" style={{ minHeight: '235px' }}>
                {post.featuredImage && (
                  // eslint-disable-next-line @next/next/no-img-element
          ...
          {remaining.map((post: any) => (
            <Link key={post.slug} href={`/storyline/${post.slug}`}
              ...
              <div className="min-w-0">
                ...
                <h3 className="font-display font-light text-sm text-line-text group-hover:text-line-accent transition-colors leading-snug truncate" style={{ letterSpacing: '-0.01em' }}>
                  {post.title}
                </h3>
              </div>
            </Link>
